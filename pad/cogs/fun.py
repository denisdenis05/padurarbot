...
class Fun(commands.Cog):
    ...
    # @commands.command()
    # async def fazan(self, ctx):
    # litere=['a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z']
    # inchis=['nt','ee','rb','ns','rt','nc','rd','lt','ft','ps','nţ','ng','lm','nz','ăr','ct','rţ','ât','rh']
    # litera=random.choice(litere)
    # print(litera)
    # embed = discord.Embed(title=f"Hai, jucam fazan!", description=f"Spuneti un cuvant cu litera `{litera}`", color=default_color)
    # await ctx.reply(embed=embed)
    # def check2(m):
    # return m.author.id == ctx.author.id and m.channel.id == ctx.channel.id
    # msg = await self.client.wait_for('message', check=check2, timeout=40)
    # if((str(msg.content).lower()).startswith(litera)):
    # ultim=str(msg.content).lower()[2:]
    # if str(ultim) in inchis:
    # embed = discord.Embed(title=f"N-ai voie sa inchizi din prima ma", description=f"Scrie alt cuvant", color=default_color)
    # ok=1
    # while ok==1:
    # ok=0
    # msg = await self.client.wait_for('message', check=check2, timeout=40)
    ...
    # embed = discord.Embed(title=f"Ai inchis frate", description=f"Bv", color=default_color)
    # await msg.reply(embed=embed)
    # if
    ...
    async def birthday(self, ctx, day=None, month=None, year: int = 0):
        ...
        try:
            ...
            if f"birthdaydate{member.id}" not in data:
                embed = discord.Embed(title="Wtf bro",
                                      description=f"Se pare ca {member} nu si-a setat ziua de nastere!",
                                      color=default_color)
                await ctx.reply(embed=embed)
                return
            else:
                ...
                if int(year) == 0:
                    ...
                    year = "." + year
                embed = discord.Embed(title="Nice!", description=f"{member} s-a nascut pe **{day}.{month}{year}**",
                                      color=default_color)
                await ctx.reply(embed=embed)
                return
        except Exception as ex:
        ...
        if day == None:
            embed = discord.Embed(title=f"Ce doresti sa faci?",
                                  description=f"Reactioneaza mai jos pentru:\n\n👁️: a vedea ziua ta de nastere\n🇸: a seta data ta de nastere\n⛔: a renunta",
            ...
            def check1(reaction, user):
                return reaction.message.id == mesaj.id and user.id == ctx.author.id and (
                            str(reaction.emoji) == s or str(reaction.emoji) == eye or str(
                        reaction.emoji) == ruler or str(reaction.emoji) == nu)
            ...
            try:
                ...
                if str(reaction.emoji) == eye:
                    if f"birthdaydate{ctx.author.id}" not in data:
                        embed = discord.Embed(title="Wtf bro",
                                              description=f"Se pare ca nu ti-ai setat ziua de nastere!",
                                              color=default_color)
                        await ctx.reply(embed=embed)
                        return
                    else:
                        ...
                        if int(year) == 0:
                            ...
                            year = "." + year
                        embed = discord.Embed(title="Nice!", description=f"Te-ai nascut pe **{day}.{month}{year}**",
                                              color=default_color)
                        await ctx.reply(embed=embed)
                        return
                    ...
                    embed = discord.Embed(title="Ok, spune-mi data ta de nastere!",
                                          description=f"Exemple: 17 Ianuarie, 20 Aprilie 1889",
                                          color=default_color)
                    ...
                    try:
                        ...
                        with open("pad/data/data.json", "w") as jsonFile:
                            ...
                            jsonFile.close()
                        embed = discord.Embed(title="Gata frate.", description=f"Voi tine minte",
                                              color=default_color)
                        await ctx.reply(embed=embed)
                        return
    ...
    @commands.command()
    async def coaie(self, ctx):
        await ctx.reply("Ai 2 coaie")
    ...
    async def urbandictionary(self, ctx, *, term):
        ...
        async with ClientSession() as session:
            async with session.get(url, headers=headers, params=querystring) as response:
                ...
                definition = definition.replace("]", "*")
                embed = discord.Embed(title=f"Urban Dictionary", description=f"Primul rezultat:",
                                      color=discord.Color.from_rgb(0, 191, 255))
                embed.set_thumbnail(
                    url='https://slack-files2.s3-us-west-2.amazonaws.com/avatars/2018-01-11/297387706245_85899a44216ce1604c93_512.jpg')
    ...
    async def clan(self, ctx, *, clan=None):
        ...
        if clan == None:
            if (f"detinatorclan{ctx.author.id}" not in data or data[f"detinatorclan{ctx.author.id}"] == 0) and (
                ...
                await ctx.send(
                    f"Nu esti intr-un clan. Foloseste comanda `.help clan` pentru a afla mai multe despre comanda.")
                return
            if f"detinatorclan{ctx.author.id}" in data and data[f"detinatorclan{ctx.author.id}"] != 0:
        ...
        embed = discord.Embed(title="DETALII CLAN", description=f"Nume clan: `{numeclan}`", color=default_color)
        try:
    ...
    async def tttest(self, ctx):
        embed = discord.Embed(
            title="Ceva", description="[ceva](https://youtube.com)", color=default_color)
        await ctx.send(embed=embed)
    ...
    async def choose(self, ctx, *, alegeri):
        try:
            ...
            return
        random = randint(1, 2)
        embed = discord.Embed(title="", description=f"", color=default_color)
        if random == 1:
    ...
    async def trivia(self, ctx):
        with open("pad/data/data.json", "r") as jsonFile:
            data = json.load(jsonFile)
            jsonFile.close()
        intrebari = {"Al cui caine este Scooby Doo?": "Freddie,Shaggy,Daphne,Velma",
                     "Ale cui erau caprele de la care Nica a luat raie?": "Smarandei,Matusei Marioara,Irinucai,Isabelei",
                     "Amnezia este pierderea . . .": "poftei de mancare,auzului,vederii,memoriei",
                     "Aproximativ in ce procent se gaseste Oxigenul in aer?": "21%,15%,16%,75%",
                     "Cand a avut loc revolutia in urma careia a fost inlaturat Nicolae Ceausescu?": "decembrie 1990,decembrie 1989,decembrie 1991,decembrie 1988",
                     "Care a fost capitala Imperiului Bizantin ?": "Roma,Ierusalim,Atena,Constantinopol",
                     "Care a fost a treia proba pe care Aleodor imparat a trebui sa o treaca pentru fata lui Verde Imparat?": "sa separe graul de orz,sa aduca capul omului Span,sa se ascunda de ea,sa o gaseasca",
                     "Care a fost numele initial al lui Muhammad Ali inainte sa treaca la islamism?": "Cassius Clay,Muhammad John,Sam Longford,Joe Louis",
                     "Care a fost prima persoana care a pasit pe luna?": "Yuri Gagarin,Neil Armstrong,Edward H. White,niciuna",
                     "Care a fost prima persoana care a pasit pe marte?": "Yuri Gagarin,Neil Armstrong,Edward H. White,niciuna",
                     "Care a fost prima persoana ce a facut inconjurul lumii?": "Amerigo Vespuci,Jules Verne,Phileas Fogg,Ferdinand Magellan",
                     "Care a fost primul aparat care putea inregistra si reproduce sunetele?": "casetofonul,fonograful,gramofonul,diapazonul",
                     "Care a fost primul model de masina Dacia?": "Dacia 100,Dacia1310,Dacia 1100,Dacia 1001",
                     "Care a fost profesorul de limba romana a lui Mihai Eminescu?": "Iosif Vulcan,Titu Maiorescu,Matei Millo,Aron Pumnul",
                     "Care din personajele romanului Ion s-a spazurat?": "Ana,Florica,Ion,George",
                     "Care din urmatoarele cuvinte este o forma corecta?": "amindurora,amindurora,amindurura,amandoura",
                     "Care din urmatoarele cuvinte este un sinonim pentru “solomonit”?": "fastuos,morocanos,dizolvabil,fermecat",
                     "Care din urmatoarele elemente nu este o ramura a biologiei?": "Virusologie,Biochimie,Micologie,Zoologie",
                     "Care din urmatoarele personaje de animatie nu au fost creatia lui Joseph Barbera?": "Scooby Doo,Familia Flinstone,Bambi,The Jetsons",
                     "Care din urmatoarele personaje nu apartin operei Vrajitorul din oz": "Geppetto,Dorothy Gale,Totto,Ozma",
                     "Care din urmatoarele personaje nu apartine filmului Mica sirena?": "Regele Triton,Ursula,Orland,Scuttle",
                     "Care din urmatoarele personaje nu face parte din “Povestea lui Harap-Alb”?": "Omul Span,Verde-Imparat,Omul Ros,Imparatul de la Miazanoapte",
                     "Care din urmatoarele personaje nu face parte din nuvela “Padureanca”?": "Simona,Busuioc,Iorgovan,Sofron",
                     "Care din urmatoarele personaje nu face parte din romanul “La Medeleni”?": "Olguta,Daut,Alina,Monica",
                     "Care dintre urmatoarele planete NU este numita “superioara”?": "Mercur,Jupiter,Neptun,Uranus",
                     "Ce este Modemul?": "Modulator/Demodulator,Modulator,Megaondulator,Miniondulator"}
        raspunsuri = {"Al cui caine este Scooby Doo?": 2, "Ale cui erau caprele de la care Nica a luat raie?": 3,
                      "Amnezia este pierderea . . .": 4, "Aproximativ in ce procent se gaseste Oxigenul in aer?": 1,
                      "Cand a avut loc revolutia in urma careia a fost inlaturat Nicolae Ceausescu?": 2,
                      "Care a fost capitala Imperiului Bizantin ?": 4,
                      "Care a fost a treia proba pe care Aleodor imparat a trebui sa o treaca pentru fata lui Verde Imparat?": 3,
                      "Care a fost numele initial al lui Muhammad Ali inainte sa treaca la islamism?": 1,
                      "Care a fost prima persoana care a pasit pe luna?": 2,
                      "Care a fost prima persoana care a pasit pe marte?": 4,
                      "Care a fost prima persoana ce a facut inconjurul lumii?": 4,
                      "Care a fost primul aparat care putea inregistra si reproduce sunetele?": 2,
                      "Care a fost primul model de masina Dacia?": 3,
                      "Care a fost profesorul de limba romana a lui Mihai Eminescu?": 4,
                      "Care din personajele romanului Ion s-a spazurat?": 1,
                      "Care din urmatoarele cuvinte este o forma corecta?": 2,
                      "Care din urmatoarele cuvinte este un sinonim pentru solomonit?": 4,
                      "Care din urmatoarele elemente nu este o ramura a biologiei?": 2,
                      "Care din urmatoarele personaje de animatie nu au fost creatia lui Joseph Barbera?": 3,
                      "Care din urmatoarele personaje nu apartin operei Vrajitorul din oz": 1,
                      "Care din urmatoarele personaje nu apartine filmului “Mica sirena”?": 3,
                      "Care din urmatoarele personaje nu face parte din “Povestea lui Harap-Alb”?": 4,
                      "Care din urmatoarele personaje nu face parte din nuvela Padureanca?": 1,
                      "Care din urmatoarele personaje nu face parte din romanul “La Medeleni”?": 3,
                      "Care dintre urmatoarele planete NU este numita “superioara”?": 1, "Ce este Modemul?": 1}
        n = randint(1, len(intrebari))
        i = 1
        for key in intrebari.items():
            if i == n:
                embed = discord.Embed(title="Trivia", description="Alege rapsunul corect! Ai 15 secunde",
                                      color=default_color)
    ...
    async def porumbel(self, ctx, member: discord.Member = None, *, text=None):
        ...
        if cuvant.upper() == "(ANONIM)":
            embed = discord.Embed(
                title="Un anonim iti transmite:", description=text[:-len(cuvant)], color=default_color)
            embed.set_footer(
                text="Poti folosi si tu comanda .porumbel + mesaj (lasa '(Anonim)' la sfarsitul mesajului daca vrei sa fi anonim")
            await member.send(embed=embed)
        else:
            embed = discord.Embed(
                title=f"{ctx.author.name} iti transmite:", description=text, color=default_color)
            embed.set_footer(
                text="Poti folosi si tu comanda .porumbel + mesaj (lasa '(Anonim)' la sfarsitul mesajului daca vrei sa fi anonim")
            await member.send(embed=embed)
    ...
    async def pup(self, ctx, member: discord.Member = None):
        ...
        if voterandom == 3:
            ...
            view=Buttons()
            view.add_item(discord.ui.Button(label="Voteaza-ma pe top.gg!",style=discord.ButtonStyle.link,url="https://top.gg/bot/790607817128017920",emoji="<a:bucimesi:960985978972033054>"))
            msg = await ctx.reply(embed=embed, view=view)
        else:
            await ctx.reply(embed=embed)

    # @commands.command(aliases=['x0','tictactoe'])
    # async def xo(self, ctx, membru: discord.Member = None):
    # gol='⬛'
    # x='❌'
    # o='⭕'
    # rand1={'⬛','⬛','⬛'}
    ...
    # rand3={'⬛','⬛','⬛'}
    # if membru == None:
    # await ctx.send("Cu cine te joci? Ruleaza iar comanda")
    # return
    # if membru.id == ctx.author.id:
    # await ctx.send("Naw bro, 1v1 nu te bati singur.")
    # return
    # await ctx.send(
    # f"{membru.mention}, esti provocat la duel de catre {ctx.author.mention}. Accepti acest meci de x si 0? Ai 15 secunde sa raspunzi. (scrie DA sau NU)")
    ...
    # return
    # if ("DA" not in msg.content.upper() and "YES" not in msg.content.upper()) and ("NU" not in msg.content.upper() and "NO" not in msg.content.upper()):
    ...
    # embed = discord.Embed(title="X SI 0 ", description="lmao wtf alege de mai jos", color=default_color)
    ...
    async def fighttop(self, ctx, nr: int = 1):
        ...
        if len(sort) < j:
            j = len(sort)
        embed = discord.Embed(title="", description="", color=default_color)
        embed.set_author(name="Topul p*nisurilor",
    ...
    async def sex(self, ctx, member: discord.Member = None):
        ...
        if member == None:
            if f"Casatorit{ctx.author.id}" in data:
                ...
                return
        prob = randint(1, 8)
        if f"Casatorit{ctx.author.id}" in data and int(data[f"Casatorit{ctx.author.id}"]) != 0 and int(
        ...
        if voterandom == 3:
            ...
            msg = await ctx.reply(embed=embed, components=[
                Button(style=5, label="Voteaza-ma pe top.gg!", url="https://top.gg/bot/885503634710884412",
                       custom_id="vot1")])
    ...
    async def marry(self, ctx, member: discord.Member = None, *, mesaj=None):
        ...
        if data[f"Casatorit{ctx.author.id}"] != 0:
            embed = discord.Embed(title="Calm ba playboy", description="Esti casatorit deja",
                                  color=default_color)
            embed.set_footer(text=ctx.author, icon_url=ctx.author.avatar)
            embed.timestamp = discord.utils.utcnow()
            await ctx.reply(embed=embed)
        ...
        if data[f"Casatorit{member.id}"] != 0:
            embed = discord.Embed(title="Calm ba playboy", description="E casatorit deja", color=default_color)
            embed.set_footer(text=ctx.author, icon_url=ctx.author.avatar)
            embed.timestamp = discord.utils.utcnow()
            await ctx.reply(embed=embed)
            ctx.command.reset_cooldown(ctx)
        ...
        if data[f"Cer{ctx.author.id}"] == member.id:
            ...
            with open("pad/data/data.json", "w") as jsonFile:
                json.dump(data, jsonFile)
                jsonFile.close()
            embed = discord.Embed(title="FELICITARI", description="Sunteti casatoriti!!!!!!!",
                                  color=discord.Color.from_rgb(255, 182, 193))
            embed.set_footer(text=ctx.author, icon_url=ctx.author.avatar)
            embed.timestamp = discord.utils.utcnow()
            embed.set_thumbnail(
                url='https://media.discordapp.net/attachments/745384647885848594/948598560180539432/916794662335950928.gif')
            await ctx.reply(embed=embed)
        else:
            data[f"Cer{member.id}"] = ctx.author.id
            embed = discord.Embed(title="Ai putina rabdare!", description="I-am trimis o cerere in casatorie.",
                                  color=discord.Color.from_rgb(255, 182, 193))
            embed.set_footer(text=ctx.author, icon_url=ctx.author.avatar)
            embed.timestamp = discord.utils.utcnow()
            embed.set_thumbnail(
                url='https://media.discordapp.net/attachments/745384647885848594/948597675064954931/940192094981611521.gif')
            await ctx.reply(embed=embed)
            with open("pad/data/data.json", "w") as jsonFile:
                json.dump(data, jsonFile)
                jsonFile.close()
...
Pentru a accepta cererea , scrie .casatorie @{ctx.author} pe serverul `{ctx.guild.name}`.""")
    ...
    async def familytree(self, ctx, member: discord.Member = None, ):
        ...
        if f"parinte{member.id}" in data and int(data[f"parinte{member.id}"]) != 0:
            ...
            avatar1 = parinte1.avatar.replace(size=128)
            dataa = BytesIO(await avatar1.read())
            pfp = Image.open(dataa)
            pfp = pfp.resize((155, 155))
            ...
            if f"Casatorit{parinte1.id}" in data and int(data[f"Casatorit{parinte1.id}"]) != 0:
                ...
                avatar1 = parinte2.avatar.replace(size=128)
                dataa = BytesIO(await avatar1.read())
                pfp = Image.open(dataa)
                pfp = pfp.resize((155, 155))
                ...
                if f"parinte{parinte2.id}" in data and int(data[f"parinte{parinte2.id}"]) != 0:
                    ...
                    avatar1 = bunic1.avatar.replace(size=128)
                    dataa = BytesIO(await avatar1.read())
                    pfp = Image.open(dataa)
                    ...
                    if f"Casatorit{bunic1.id}" in data and int(data[f"Casatorit{bunic1.id}"]) != 0:
                        ...
                        avatar1 = bunic2.avatar.replace(size=128)
                        dataa = BytesIO(await avatar1.read())
                        pfp = Image.open(dataa)
                        pfp = pfp.resize((140, 140))
            ...
            if f"parinte{parinte1.id}" in data and int(data[f"parinte{parinte1.id}"]) != 0:
                ...
                avatar1 = bunic3.avatar.replace(size=128)
                dataa = BytesIO(await avatar1.read())
                pfp = Image.open(dataa)
                pfp = pfp.resize((140, 140))
                ...
                if f"Casatorit{bunic3.id}" in data and int(data[f"Casatorit{bunic3.id}"]) != 0:
                    ...
                    avatar1 = bunic4.avatar.replace(size=128)
                    dataa = BytesIO(await avatar1.read())
                    pfp = Image.open(dataa)
                    pfp = pfp.resize((140, 140))
    ...
    async def sot(self, ctx, member: discord.Member = None):
        ...
        if (data[f"Casatorit{member.id}"] == 0):
            if member.id == ctx.author.id:
                embed = discord.Embed(title="Esti singur domnule", description="Nu esti casatorit",
                                      color=default_color)
                embed.set_footer(text=ctx.author, icon_url=ctx.author.avatar)
                embed.timestamp = discord.utils.utcnow()
                embed.set_thumbnail(
                    url='https://media.discordapp.net/attachments/745384647885848594/948597179033985114/924645878193586186.gif')
                await ctx.reply(embed=embed)
            else:
                embed = discord.Embed(title="Este un domn singur", description=f"{member.mention} nu e casatorit",
                                      color=default_color)
                embed.set_footer(text=ctx.author, icon_url=ctx.author.avatar)
                embed.timestamp = discord.utils.utcnow()
                embed.set_thumbnail(
            ...
            if member.id == ctx.author.id:
                embed = discord.Embed(title="Felicitari!",
                                      description=f"{member.mention}, esti intr-o relatie cu {persoana} de {timp}",
                                      color=discord.Color.from_rgb(255, 182, 193))
                embed.set_footer(text=ctx.author, icon_url=ctx.author.avatar)
                embed.timestamp = discord.utils.utcnow()
                embed.set_thumbnail(
                    url='https://images-ext-2.discordapp.net/external/zy5gZQOkf6fez6tOPeAtvZomGwlEPFt2PxTVfNWEBAM/%3Fv%3D1%26size%3D40/https/cdn.discordapp.com/emojis/784630478673936434.gif')
                await ctx.reply(embed=embed)
            else:
                embed = discord.Embed(title="Frumos!",
                                      description=f"{member.mention} e intr-o relatie cu {persoana} de {timp}",
                                      color=discord.Color.from_rgb(255, 182, 193))
                embed.set_footer(text=ctx.author, icon_url=ctx.author.avatar)
                embed.timestamp = discord.utils.utcnow()
                embed.set_thumbnail(
    ...
    async def divort(self, ctx):
        ...
        with open("pad/data/data.json", "w") as jsonFile:
            ...
            jsonFile.close()
        if data[f"Casatorit{ctx.author.id}"] == 0:
            embed = discord.Embed(title="Uhmmmm", description=f"Nu esti casatorit.", color=default_color)
            embed.set_footer(text=ctx.author, icon_url=ctx.author.avatar)
            embed.timestamp = discord.utils.utcnow()
            embed.set_thumbnail(
                url='https://www.freepnglogos.com/uploads/question-mark-png/file-red-question-mark-svg-wikimedia-commons-14.png')
            await ctx.reply(embed=embed)
        else:
            ...
            with open("pad/data/data.json", "w") as jsonFile:
                json.dump(data, jsonFile)
                jsonFile.close()
            embed = discord.Embed(title="Gata documentele", description=f"Nu mai esti casatorit!!",
                                  color=default_color)
            embed.set_footer(text=ctx.author, icon_url=ctx.author.avatar)
            embed.timestamp = discord.utils.utcnow()
            embed.set_thumbnail(
                url='https://images-ext-2.discordapp.net/external/xp1mvFwAseXgYV4UCIgZE4wteLIdvM5x3PuyEwMx0YE/%3Fv%3D1%26size%3D40/https/cdn.discordapp.com/emojis/903543737534283836.gif')
            await ctx.reply(embed=embed)
