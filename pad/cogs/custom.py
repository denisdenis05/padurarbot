...
class Custom(commands.Cog):
    ...
    @commands.command()
    async def testbut(self, ctx):
      msg = await ctx.reply("cee", components=[Button(style=3, label="Voteaza-ma pe top.gg!", custom_id="testlmao")])

    @commands.command()
    async def testbuton(self, ctx):
        msg = await ctx.send(
            "test", components=[
                Button(style=5, label="Voteaza-ma pe top.gg!", url="https://top.gg/bot/790607817128017920",
                       custom_id="vot")])
        while ctx.message != None:
            try:
                ...
                await msg.edit("test", components=[])
                return
